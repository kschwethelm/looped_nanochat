...
parser.add_argument(
    "--split-tokens", type=int, default=40 * 524288, help="number of tokens to evaluate per split"
)
parser.add_argument(
...
parser.add_argument(
    "--hf-path", type=str, default=None, help="HuggingFace model path (e.g. openai-community/gpt2)"
)
    ...
    for prompt in prompts:
        ...
        with autocast_ctx:
            sample, _ = engine.generate_batch(
                tokens, num_samples=1, max_tokens=16, temperature=0, num_recur=None
            )
...
if ddp_rank == 0 and args.hf_path is None:
    ...
    with autocast_ctx:
        samples, _ = engine.generate_batch(
            tokens, num_samples=8, max_tokens=128, temperature=1.0, num_recur=None
        )
