        ...
        if rank == 0:
            for group in param_groups:
                ...
                assert isinstance(group["params"], list), (
                    "expecting group['params'] to be a list of tensors"
                )
                for p in group["params"]:
                    ...
                    if sliced:  # large parameter tensors will be operated on in slices
                        assert p.shape[0] % world_size == 0, (
                            f"First dim of parameter shape {p.shape} must be divisible by world size {world_size}"
                        )
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                if p.numel() < 1024:
                    ...
                    rank_size = (
                        grad.shape[0] // world_size
                    )  # p.shape[0] % world_size == 0 is checked in __init__
